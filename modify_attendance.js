...
// 2. Add User and Briefcase to imports
code = code.replace(
...
// 4. Wrap the existing content in a check for student tab
code = code.replace(
  "      {(user?.role as string) === 'student' ? (",
  "      {activeTab === 'student' && ((user?.role as string) === 'student' ? ("
);

// Make sure the end of the student tab is closed properly.
// The file ends with:
//         </>
//       )}
//     </div>
//   );
// }
// We need to add the Faculty tab rendering there.

const facultyTabHtml = `
