  ...
  return (
    <div className="min-h-screen bg-slate-950 flex overflow-hidden font-sans selection:bg-primary/30">
      ...
      <AnimatePresence mode="wait">
        {isSidebarOpen && (
          <motion.aside 
            ...
            className="w-72 bg-slate-900/50 backdrop-blur-xl border-r border-white/10 h-screen flex flex-col fixed left-0 z-50 shadow-2xl shadow-black/50"
          >
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.name} href={item.href}>
                    <div className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-300 ${isActive ? 'bg-primary/90 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'hover:bg-white/5 text-white/60 hover:text-white'}`}>
            ...
            <div className="p-4 mt-auto border-t border-white/10">
              <button 
                onClick={logout}
                className="flex items-center gap-4 px-4 py-3.5 w-full rounded-xl hover:bg-red-500/10 text-white/60 hover:text-red-400 transition-colors duration-300"
              >
      ...
      <div className={`flex-1 transition-all duration-300 z-10 flex flex-col ${isSidebarOpen ? 'lg:ml-72' : 'ml-0'}`}>
        <header className="h-20 bg-slate-900/30 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            {!isSidebarOpen && (
              <button onClick={() => setIsSidebarOpen(true)} className="p-2 rounded-lg hover:bg-white/10 text-white/70 transition-colors">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-primary to-purple-600 border border-white/20 shadow-lg cursor-pointer flex items-center justify-center text-white font-bold text-sm hover:scale-105 transition-transform">
