...
function ShootingStar() {
  ...
  const speed = useRef(Math.random() * 15 + 15);
  const delay = useRef(Math.random() * 8);
  const timer = useRef(0);
  
  // Angle for the shooting star (almost straight down, like rain/meteor shower)
  const angle = useRef(Math.PI / 2 + (Math.random() * 0.4 - 0.2));
